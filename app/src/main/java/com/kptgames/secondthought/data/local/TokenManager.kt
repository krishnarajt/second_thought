...
class TokenManager(private val context: Context) {
    ...
    companion object {
        ...
        private val TELEGRAM_LINKED_KEY = booleanPreferencesKey("telegram_linked")
    }
    ...
    // Save Telegram linked status
    suspend fun saveTelegramLinked(isLinked: Boolean) {
        context.dataStore.edit { preferences ->
            preferences[TELEGRAM_LINKED_KEY] = isLinked
        }
    }

    // Get Telegram linked status
    fun getTelegramLinked(): Flow<Boolean> = context.dataStore.data.map { preferences ->
        preferences[TELEGRAM_LINKED_KEY] ?: false
    }
}